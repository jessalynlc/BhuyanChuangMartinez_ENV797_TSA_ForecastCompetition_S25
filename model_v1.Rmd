...
ts_rh <- ts(relative_humidity_vector, frequency = 365.25)
ts_temp <- ts(temperature_vector, frequency = 365.25)
rh_forecast <- forecast(auto.arima(ts_rh), h = 59)
temp_forecast <- forecast(auto.arima(ts_temp), h = 59)
future_rh <- as.numeric(rh_forecast$mean)
future_temp <- as.numeric(temp_forecast$mean)
